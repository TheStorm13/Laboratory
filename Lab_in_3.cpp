...
bool is_magic_matrix(std::vector<std::vector<int>> matrix) {
    ...
    for (int i = 0; i < matrix.size(); ++i) {
        for (int j = 0; j < matrix.size(); ++j) {
            ...
            if (i == j) sum_matrix[2 * matrix.size()] += matrix[i][j];
            if (i + j == matrix.size() - 1) sum_matrix[2 * matrix.size() + 1] += matrix[i][j];
        }
    }
    for (int k = 0; k < sum_matrix.size() - 1; ++k) flag *= (sum_matrix[k] == sum_matrix[k + 1]);
    return flag;
}
...
void print(std::vector<std::vector<int>> matrix, bool flag) {
    ...
    if (flag == 1) std::cout << std::endl << "This is a magic matrix" << std::endl;
    else std::cout << std::endl << "This is not a magic matrix" << std::endl;
