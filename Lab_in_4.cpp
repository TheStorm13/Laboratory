...
std::vector<char> read_file(std::string name_file,std::vector<char> array) {
    ...
    if (file.is_open()) {
        while (file.get(symbol)) array.push_back(symbol);
    }
    file.close();
    return array;
}
...
std::vector<char> string_conversion(std::vector<char> array) {
    for (int i = 0; i < array.size(); ++i) {
        if (isalpha(array[i])) {
            if (isalpha(array[i - 1]) == 0 || i == 0) (array[i]) = toupper(array[i]);
            else (array[i]) = tolower(array[i]);
...
void record_file(std::string name_file,std::vector<char> array) {
    ...
    if (file.is_open()) {
        for (char symbol: array) file << symbol;
    }
    file.close();
